...
- Keep Python 3.8 compatibility (no `|` unions, no `match`).
...
```
Project: FlaUI Python wrapper (1:1 C# API parity). Call setup_pythonnet_bridge() first. Use @handle_csharp_exceptions on interop. Python 3.8 typing. Use ConditionFactory / XPath. Post-wait support on input. Fixtures provide matrix UIA2/UIA3 × WinForms/WPF.
```

## Common mistakes to avoid
- Importing C# assemblies before `setup_pythonnet_bridge()`.
- Skipping exception decorator on interop methods.
- Using 3.9+ syntax (`|` unions, match/case).
