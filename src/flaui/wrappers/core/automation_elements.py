...
class AutomationElement:
    ...
    def __init__(self, element: Any) -> None:
        ...
        self.condition_factory: Any = self.element.ConditionFactory #   Shortcut to the condition factory for the current automation.
        ...
        self.framework_automation_element: Any = self.element.FrameworkAutomationElement    #   Object which contains the native wrapper element (UIA2 or UIA3) for this element.
        self.framework_type: Any = self.element.FrameworkType   #   The direct framework type of the element. Results in "FrameworkType.Unknown" if it couldn't be resolved.
        self.help_text: str = self.element.HelpText #   The help text of this element.
        self.is_available: bool = self.element.IsAvailable  #    A flag that indicates if the element is still available. Can be false if the element is already unloaded from the ui.
    ...
    def return_automation_element(self, element) -> AutomationElement:
    ...
    def find_all(self, tree_scope: Any, condition: Optional[Any]) -> List[AutomationElement]:
        ...
        """
        return [self.return_automation_element(_) for _ in TypeConverter.cast_to_py_list(self.element.FindAll(tree_scope, condition))]
    ...
    def find_all_by_x_path(self, x_path: str) -> List[AutomationElement]:
        ...
        :return: The found elements or an empty list if no elements were found.
        """
        return [self.return_automation_element(_) for _ in TypeConverter.cast_to_py_list(self.element.FindAllByXPath(x_path))]
    ...
    def find_all_children(self, condition: Optional[Any]) -> List[AutomationElement]:
        ...
        :return: The found elements or an empty list if no elements were found.
        """
        return [self.return_automation_element(_) for _ in TypeConverter.cast_to_py_list(self.element.FindAllChildren(condition))]
    ...
    def find_all_descendants(self, condition: Optional[Any]) -> List[AutomationElement]:
        ...
        :return: The found elements or an empty list if no elements were found.
        """
        return [self.return_automation_element(_) for _ in TypeConverter.cast_to_py_list(self.element.FindAllDescendants(condition))]
    ...
    def find_all_nested(self, condition: Optional[Any]) -> List[AutomationElement]:
        ...
        :return: The found elements or an empty list if no elements were found.
        """
        return [self.return_automation_element(_) for _ in TypeConverter.cast_to_py_list(self.element.FindAllNested(condition))]

    def find_all_with_options(self, tree_scope: Any, condition: Optional[Any], traversal_options: Any, root: Any) -> List[AutomationElement]:
        ...
        :return: The found elements or an empty list if no elements were found.
        """
        return [self.return_automation_element(_) for _ in TypeConverter.cast_to_py_list(self.element.FindAllWithOptions(tree_scope, condition, traversal_options, root))]
    ...
    def find_first_with_options(self, tree_scope: Any, condition: Optional[Any], traversal_options: Any, root: Any) -> AutomationElement:
        ...
        :return: The found element or null if no element was found.
        """
        return self.return_automation_element(self.element.FindFirstWithOptions(tree_scope, condition, traversal_options, root))
...
class ComboBox(AutomationElement):
    ...
    def __init__(self, element: Any) -> None:
        ...
        self.editable_text: str = self.element.EditableText  #   The text of the editable element inside the combobox. Only works if the combobox is editable.
        self.is_editable: bool = self.element.IsEditable #   Flag which indicates, if the combobox is editable or not.
        self.is_read_only: bool = self.element.IsReadOnly    #   Flag which indicates, if the combobox is read-only or not.
        self.value: str = self.element.Value #   Getter / setter for the selected value.
        self.selected_items: List[str] = TypeConverter.cast_to_py_list(self.element.SelectedItems)   #   Gets all selected items.
        ...
        self.expand_collapse_state: Any = self.element.ExpandCollapseState  #   Gets the ExpandCollapseStateof the element.
...
class DataGridView(AutomationElement):
    ...
    def __init__(self, element: Any) -> None:
        ...
        self.has_add_row: bool = self.element.HasAddRow #   Flag to indicate if the grid has the "Add New Item" row or not. This needs to be set as FlaUI cannot find out if this is the case or not.
        self.header: DataGridViewHeader = DataGridViewHeader(self.element.Header)  #   Gets the header element or null if the header is disabled.
        self.rows: List[DataGridViewRow] = [DataGridViewRow(_) for _ in TypeConverter.cast_to_py_list(self.element.Rows)] #   Gets all the data rows.
...
class DateTimePicker(AutomationElement):
    ...
    def __init__(self, element: Any) -> None:
        ...
        self.selected_date = self.element.SelectedDate  #   For Win32, setting SelectedDate to null will uncheck the DateTimePicker control and disable it. Also for Win32, if the control is unchecked then SelectedDate will return null.
...
class Grid(AutomationElement):
    ...
    def __init__(self, element: Any) -> None:
        ...
        self.column_headers: List[AutomationElement] = [self.return_automation_element(_) for _ in TypeConverter.cast_to_py_list(self.element.ColumnHeaders)]   #   Gets all column header elements.
        self.row_headers: List[AutomationElement] = [self.return_automation_element(_) for _ in TypeConverter.cast_to_py_list(self.element.RowHeaders)]   #   Gets all row header elements.
        self.row_or_column_major: Any = self.element.RowOrColumnMajor   #   Gets whether the data should be read primarily by row or by column.
        self.header: Any = self.element.Header  #   Gets the header item
        self.rows: List[Any] = [_ for _ in TypeConverter.cast_to_py_list(self.element.Rows)]    #   Returns the rows which are currently visible to UIA. Might not be the full list (eg. in virtualized lists)! Use GetRowByIndex to make sure to get the correct row.
        self.selected_items: List[Any] = [_ for _ in TypeConverter.cast_to_py_list(self.element.SelectedItems)] #   Gets all selected items.
    ...
    def add_to_selection(self, row_index: Optional[int]=None,column_index: Optional[int]=None, text_to_find: Optional[str]=None) -> Any:
    ...
    def remove_from_selection(self, row_index: Optional[int]=None,column_index: Optional[int]=None, text_to_find: Optional[str]=None) -> Any:
...
class ListBox(AutomationElement):
    ...
    def __init__(self, element: Any) -> None:
        ...
        self.items: List[ListBoxItem] = TypeConverter.cast_to_py_list(self.element.Items)  #   Returns all the list box items
...
class ListBoxItem(AutomationElement):
    ...
    def __init__(self, element: Any) -> None:
        ...
        self.is_checked: bool = self.element.IsChecked  #   Gets or sets if the listbox item is checked, if checking is supported
        self.text: str = self.element.Text  #   Gets the text of the element.
...
class Menu(AutomationElement):
    ...
    def __init__(self, element: Any) -> None:
        ...
        self.is_win_menu = self.element.IsWin32Menu #   Flag to indicate if the menu is a Win32 menu because that one needs special handling
...
class MenuItem(AutomationElement):
    ...
    def __init__(self, element: Any) -> None:
        ...
        self.is_win_menu = self.element.IsWin32Menu #   Flag to indicate if the containing menu is a Win32 menu because that one needs special handling
...
class Slider(AutomationElement):
    ...
    def __init__(self, element: Any) -> None:
        ...
        self.large_increase_button: Any = self.element.LargeIncreaseButton    #   The button element used to perform a large increment.
        self.large_decrease_button: Any = self.element.LargeDecreaseButton    #   The button element used to perform a large decrement.
        self.thumb: Any = self.element.Thumb    #   The element used to drag.
        self.is_only_value: bool = self.element.IsOnlyValue   #   Flag which indicates if the Slider supports range values (min->max) or only values (0-100). Only values are for example used when combining UIA3 and WinForms applications.
...
class Spinner(AutomationElement):
    ...
    def __init__(self, element: Any) -> None:
        ...
        self.maximum: float = self.element.Maximum  #   The maximum value.
        self.small_change: float = self.element.SmallChange  #   The value of a small change.
        self.increase_button: Any = self.element.IncreaseButton  #   The button element used to perform a large increment.
        self.decrease_button: Any = self.element.DecreaseButton  #   The button element used to perform a large decrement.
        self.is_only_value: bool = self.element.IsOnlyValue   #   Flag which indicates if the Spinner supports range values (min->max) or only values (0-100). Only values are for example used when combining UIA3 and WinForms applications.
        self.value: float = self.element.Value  #   Gets or sets the current value.
