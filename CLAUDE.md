...
- The docs site is built with **Zensical** (configured by `zensical.toml`), with API reference
  ...
  - Build: `uv run python scripts/extract_versions.py && uv run python scripts/build_changelog.py && uv run zensical build -f zensical.toml`
...
- `docs/changelog.md` is generated at build time by `scripts/build_changelog.py` (live GitHub
  Releases fetch, gitignored, build-safe offline) — never edit or commit it by hand.
